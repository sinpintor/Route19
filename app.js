 ...
 * Version: 2.2.0 (VOGUE Editorial - Seasonal Edition)
 */
...
// 1. 新增：Route19 專屬四季主題色系 (The Editorial Palettes)
const seasonalThemes = {
    "Spring": { color: "#9CA986", label: "SPRING EDITION" }, // 鼠尾草綠 (3-5月)
    "Summer": { color: "#7895A2", label: "SUMMER EDITION" }, // 愛琴海藍 (6-8月)
    "Autumn": { color: "#B37B58", label: "AUTUMN EDITION" }, // 焦糖琥珀 (9-11月)
    "Winter": { color: "#d4af37", label: "WINTER EDITION" }  // 復古鎏金 (12-2月)
};
let currentBaseColor = "#d4af37"; // 系統預設值，載入時會自動被覆蓋
...
// 2. 新增：自動感知月份，套用季節專屬色系與標題
function applySeasonalTheme() {
    const month = new Date().getMonth() + 1; // 取得目前月份 (1-12)
    let season = "Winter";
    ...
    currentBaseColor = seasonalThemes[season].color;
    
    // 全局變更 CSS 變數，整個網站的金色都會變成專屬季節色！
    document.documentElement.style.setProperty('--active-color', currentBaseColor);

    // 自動把首頁標題加上雜誌季節號 (例如：THE ROUTE N.19 GUIDE // SPRING EDITION)
    const oracleSubtitle = document.querySelector('#oracle .vogue-subtitle');
    if(oracleSubtitle) {
        oracleSubtitle.innerText = `THE ROUTE N.19 GUIDE // ${seasonalThemes[season].label}`;
    }
}
...
window.onload = () => { 
    applySeasonalTheme(); // 網頁載入時第一件事就是換季！
    ...
    cloudData.forEach(j => {
        ...
        if (idx !== -1) {
            ...
            if (stamp) { 
                ...
                // 3. 護照印章：如果有牌卡顏色就用牌卡色，沒有的話就使用目前的「季節主色」
                const stampColor = j.color || currentBaseColor; 
...
function drawFate(manualLoc = null) {
    ...
    const timer = setInterval(() => {
        ...
        if (i > 25) {
            ...
            // 4. 當抽取提案時，將介面色調變更為「被抽中的牌卡顏色」，如果沒有預設色就退回「季節色」
            document.documentElement.style.setProperty('--active-color', s1.color || currentBaseColor);
            ...
            currentRecord = { 
                ...
                color: s1.color || currentBaseColor, date: new Date().toISOString() 
            };
        }
    }, 60);
}
...
function generateIGStory() {
    ...
    // 海報同樣繼承當前的顏色
    posterTemplate.style.setProperty('--active-color', currentRecord.color || currentBaseColor);
