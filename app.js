 ...
 * Version: 2.1.1 (Classic Editorial Edition)
 */
...
window.onload = () => { 
    document.documentElement.style.setProperty('--active-color', '#d4af37'); // 確保初始載入為經典金
...
function drawFate(manualLoc = null) {
    ...
    const timer = setInterval(() => {
        ...
        if (i > 25) {
            ...
            document.documentElement.style.setProperty('--active-color', s1.color || '#d4af37');
...
function generateIGStory() {
    ...
    posterTemplate.style.setProperty('--active-color', currentRecord.color || '#d4af37');
